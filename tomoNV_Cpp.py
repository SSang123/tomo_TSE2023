...
class tomoNV_Cpp:
  ...
  def __init__(self, filename, nYPR_Intervals, yaw_range, pitch_range, roll_range, bVerbose):
    ...
    (self.mesh0, self.vtx, self.vtx_nrm, self.tri, self.tri_area, self.tri_center, self.chull_vtx) = LoadInputMesh( filename)
  ...
  def Run(self, cpp_function_name):
    ...
    self.Int32Info = np.array([   
      self.bVerbose,      self.bUseExplicitSS,  self.bUseClosedVolumeVoxel,   self.nVoxel,
      self.vtx.shape[0],  self.tri.shape[0],    self.YPR.shape[0], self.chull_vtx.shape[0],  
      self.BedType[0]]).astype( np.int32) 
    ...
    self.CppFunction.argtypes   = (  Cptr1d, Cptr1iL, Cptr1d, Cptr1d, Cptr1d, Cptr1d, Cptr1iL)
